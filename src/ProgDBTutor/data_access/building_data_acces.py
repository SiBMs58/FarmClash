...
class BuildingDataAccess:
    ...
    def get_buildings_by_username_owner(self, username_owner):
        """
        Fetches all buildings belonging to the given username_owner out of the database
        ...
        for result in results:
            building = Building(result['building_id'], result['username_owner'], result['farm_id'],
                                result['building_type'], result['level'], result['x'], result['y'],
                                result['tile_rel_locations'], result['created_at'])
    ...
    def add_building(self, building):
        """
        Adds the given building object to the database
        :param building: A building object
        :return: True if the building was added successfully, False otherwise
        """
        cursor = self.db_connection.get_cursor()
        if building.building_id:
            # Check if the building already exists with the given building_id
            cursor.execute("SELECT * FROM buildings WHERE building_id = %s", (building.building_id,))
            existing_building = cursor.fetchone()
            if existing_building:
                # Update the existing building entry
                cursor.execute("""
                    UPDATE buildings
                    SET username_owner = %s, farm_id = %s, building_type = %s, level = %s, x = %s, y = %s, 
                    tile_rel_locations = %s, created_at = %s
                    WHERE building_id = %s;
                """, (building.username_owner, building.farm_id, building.building_type, building.level,
                      building.x, building.y, building.tile_rel_locations, building.created_at, building.building_id))
            else:
                # Insert a new building entry
                cursor.execute("""
                    INSERT INTO buildings (building_id, username_owner, farm_id, building_type, level, x, y, 
                    tile_rel_locations, created_at)
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s);
                """, (building.building_id, building.username_owner, building.farm_id, building.building_type,
                      building.level, building.x, building.y, building.tile_rel_locations, building.created_at))
            self.db_connection.conn.commit()
            return True
        else:
            return False  # Building ID cannot be None
